...
- 状態（2026-09-05）: **app v0.19.0 まで完了**。5種のリズムとドラムだけモード、12キー＋ランダムのキー選択、メロディの重力、21種のスケール選択、数字キーSFX、音色4種とShift切替、
...
v0.19.0では **ASDFGHJKL の段はすべて効果なし**で、確実に素の音が出る。7音階では度数の配分を
26キー全体で均等になるよう決め、度数7だけ1キーにする。役割ごとのキー数は音階によって変わる。
隣接するタイルは同じ度数にならない（役割・度数を丸ごと入れ替えて解消。効果・オクターブは動かさない）。
配置の規則が変わったので、同じseedでもv0.18.0以前とは配置が異なる。
