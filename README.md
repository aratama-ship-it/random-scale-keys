...
- 状態（2026-09-05）: **app v0.18.0 まで完了**。5種のリズムとドラムだけモード、12キー＋ランダムのキー選択、メロディの重力、21種のスケール選択、数字キーSFX、音色4種とShift切替、
...
v0.18.0では **ASDFGHJKL の段はすべて効果なし**で、確実に素の音が出る。7音階ではタイルに「7」と出る鍵を
1キーだけにする。ただし `hijaz` は floating の度数が7だけで、上限を適用すると26キーを埋められないため例外。
隣接するタイルは同じ度数にならない（役割・度数を丸ごと入れ替えて解消。効果・オクターブは動かさない）。
配置の規則が変わったので、同じseedでもv0.17.0以前とは配置が異なる。
